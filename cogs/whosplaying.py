...
class WhosPlaying(commands.Cog):
    ...
    async def whosplaying(self, ctx, *, game):
        ...
        if playing_game == "":
            ...
            em.set_author(name=f"""Who's playing "{game}"? {showing}""", icon_url="youtube.com")
    ...
    async def currentgames(self, ctx):
        ...
        if not freq_list:
            ...
            em.set_author(name="Top games being played right now in the server:", icon_url="youtube.com")
            await ctx.send(embed=em)
